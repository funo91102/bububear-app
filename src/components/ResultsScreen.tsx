...
const ResultsScreen: React.FC = () => {
  ...
  const supportTheme = useMemo(() => {
    switch (overallStatus) {
      ...
      case 'great': // 新增：全滿分狀態 (若 assessmentEngine 有實作此狀態)
        return {
          bg: 'bg-emerald-50', 
      ...
      default: // Fallback (通常對應 normal)
        return {
          bg: 'bg-emerald-50', bgStrong: 'bg-emerald-100', border: 'border-emerald-100', text: 'text-emerald-800',
          icon: '🌟', bearEmoji: '🥳', title: '太棒了！寶寶如期達標', description: '目前的發展狀況良好，都在安全範圍內。', actionTitle: '持續保持', actionDesc: '請記得定期接種疫苗，並持續觀察寶寶的成長喔！'
        };
    }
  }, [overallStatus]);
